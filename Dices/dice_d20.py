...
class Dice:
  ...
  def create_geometry(self):
    if self.num_sides == 4:
      s = self.size
      self.vertices = [(s, s, s), (s, -s, -s), (-s, s, -s), (-s, -s, s)]
      ...
      self.faces = [(0, 1, 2, 3), (4, 5, 6, 7), (0, 1, 5, 4), (2, 3, 7, 6),
                    (0, 3, 7, 4), (1, 2, 6, 5)]
      self.normals = [
        self.calculate_normal(
          self.vertices[f[0]], self.vertices[f[1]], self.vertices[f[2]]
      ...
      self.colors = [
        ...
        GREEN,
        BLUE,
        WHITE, (1, 0.5, 0), (0, 0.5, 1), (0.5, 0, 1), (1, 1, 0)
      ]
  ...
  def generate_number_vertices(self):
    self.number_vertices = {
      ...
      '3': [(-0.5, 1), (0.5, 1), (0.5, 0), (-0.5, 0), (0.5, 0), (0.5, -1),
            (-0.5, -1)],
  ...
  def draw_numbers(self):
    ...
    for i, face in enumerate(self.faces):
      ...
      if self.num_sides == 4:
        ...
        offset = [
          n * 0.3 * self.size for n in self.normals[i % len(self.normals)]
        ]
  ...
  def update(self, dt, plane_y=-2):
    ...
    if min_distance < 0.01:
      ...
      relative_velocity = [
        self.velocity[i] + (
          ...
          self.angular_velocity[(i + 2) % 3] * local_vertex[(i + 1) % 3]
        ) for i in range(3)
      ]
      ...
      if dot_product < 0:
        ...
        # --- Трение ---
        tangent_velocity = [
          relative_velocity[i] - dot_product * normal[i] for i in range(3)
        ]
        ...
        if tangent_magnitude > 1e-6:
          ...
          friction_torque = [
            cross_product[i] * friction_impulse for i in range(3)
          ]
        ...
        kinetic_energy_linear = 0.5 * self.mass * sum(
          v**2 for v in self.velocity
        )
        kinetic_energy_angular = 0.5 * sum(
          self.inertia_tensor[i][i] * self.angular_velocity[i]**2
          for i in range(3)
        )
    ...
    if self.grounded_timer > 0.1 and self.is_stable() and all(
      abs(v) < 0.1 for v in self.velocity
    ) and all(abs(av) < 0.5 for av in self.angular_velocity):
      self.is_rolling = False
...
def handle_input():
    ...
    if event.type == QUIT:
      ...
      if event.key == K_ESCAPE:
        ...
        if not dice.is_rolling:
          initial_velocity = [
            random.uniform(-5, 5), random.uniform(8, 12), random.uniform(-5, 5)
          ]
  ...
  if flying_mode:
    ...
    forward_x = -math.sin(math.radians(camera_yaw)) * math.cos(
      math.radians(camera_pitch)
    )
    forward_y = math.sin(math.radians(camera_pitch))
    forward_z = -math.cos(math.radians(camera_yaw)) * math.cos(
      math.radians(camera_pitch)
    )
